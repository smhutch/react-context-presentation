...
import PlaygroundRaw from '!raw-loader!./examples/Playground'
...
<Surfer
  ...
  steps={[
    {},
    { range: [6,8], notes: "Initial dog state is null." },
    { lines: [14], notes: "When clicking on a Dog, we set the state." },
    { lines: [15, 16, 18, 19], notes: "We render a Modal when the dog state is set." },
    { lines: [17], notes: "And pass some data to the content of the modal." }
  ]}
/>
...
<Surfer
  ...
  steps={[
    ...
    { lines: [15, 16, 18, 19], notes: "🤔 Could we avoid rewriting this logic for each Modal?" },
    { lines: [14], notes: "🤔 Do we need to prop-drill this event through multiple components?" },
    { range: [6, 8], notes: "🤔 Does this component really need to be stateful?" },
  ]}
/>
...
<Surfer
  ...
  steps={[
    ...
    { lines: [25], notes: 'In example 2, the child component managed its own modal state'},
...
<Appear>
  <p>Let's revisit our questions from <S>Example 1</S>: Props and State</p>
</Appear>

---

<Surfer
  title="Example 1: Props and State"
  code={WithProps}
  steps={[
    {},
...
<Surfer
  title="Example 2: Context and State"
  code={WithContext}
  steps={[
